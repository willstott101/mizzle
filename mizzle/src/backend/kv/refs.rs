...
pub(super) async fn resolve_ref(
    ...
    let result: Result<Option<ObjectId>> = async {
        let value = t
...
pub(super) async fn update_refs(
    ...
    let result: Result<()> = async {
        for (old_oid, new_oid, refname) in updates {
            ...
            if delete {
                t.delete(key).await.context("delete ref")?;
            } else {
                t.put(key, new_oid.as_bytes().to_vec())
                    .await
                    .context("write ref")?;
            }
        }
    ...
    .await;
    txn::finalize_write(&mut t, result).await
}
